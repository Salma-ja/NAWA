...
const TeacherDashboard = ({ c, activeProfile, authToken, onBack }) => {
  ...
  const loadDashboard = async (searchText, nextSort) => {
    ...
    try {
      ...
      // Real backend returns a flat array of result rows — adapt to dashboard shape
      ...
      setDashboardState({
        ...
        error: "",
        summary: [
          { value: String(students.length), title: c.teacherDashboardSummaryTotal || "إجمالي النتائج", text: "" },
          {
            value: students.length ? Math.round(students.reduce((s, r) => s + r.progress, 0) / students.length) + "%" : "--",
            title: c.teacherDashboardSummaryAvg || "متوسط الإنجاز",
            text: ""
          }
        ],
        students: filtered
